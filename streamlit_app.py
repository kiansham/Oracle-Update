...
file_path = dataframe["Original Data"]
...
def aggframe():
    if "current_df" in st.session_state:
        ...
        col1.metric(
            label="Total Companies", value=f"{stats['total_filtered_companies']:,}"
        )
        col2.metric(
            label="UK Companies", value=f"{stats['total_filtered_uk_companies']:,}"
        )
        col3.metric(
        ...
        with col2:
            ...
            st.download_button(
                label="Download CSV", data=csv, file_name="filtered_data.csv"
            )
